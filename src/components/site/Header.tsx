  ...
  return (
    <header
      className={`fixed inset-x-0 top-0 z-50 transition-all duration-300 ${
        solid ? "bg-white/90 shadow-[0_1px_0_rgba(27,42,74,0.08)] backdrop-blur-md" : "bg-transparent"
      }`}
    >
      <div className="container-site flex h-[72px] items-center justify-between">
        <Link href="/" className="flex items-center gap-3" aria-label="Studio Arte Immobiliare - Home">
        ...
        <nav className="hidden items-center gap-1 md:flex" aria-label="Principale">
          ...
          <Link href="/valuta-casa" className={`${solid ? "btn-primary" : "btn-gold"} btn btn-sm ml-2`}>
            Valuta casa
          </Link>
        </nav>
      ...
      >
        <div className="flex h-full flex-col justify-between px-6 pb-10 pt-6">
          <nav className="flex flex-col gap-1" aria-label="Mobile">
            {[...NAV, { href: "/valuta-casa", label: "Valuta casa" }].map((item) => (
              <Link
                key={item.href}
                href={item.href}
          ...
          <div className="space-y-3 text-white/70">
            <a href={`tel:${AZIENDA.telefonoE164}`} className="flex items-center gap-3 text-lg text-gold">
